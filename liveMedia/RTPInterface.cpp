...
)
{
  ...
  addStreamSocket(socketDescriptor, streamChannelId);
}
...
)
{
  ...
  for (
    tcpStreamRecord* streams = fTCPStreams;
    streams != NULL;
    streams = streams->fNext
  )
  {
    if (
      streams->fStreamSocketDescriptor == socketDescriptor &&
      streams->fStreamChannelId == streamChannelId
    )
      return; // we already have it
  }

  fTCPStreams = new tcpStreamRecord(socketDescriptor, streamChannelId, fTCPStreams);
...
)
{
  ...
  while (1)
  {
    ...
    {
      if (
        (*streamsPtr)->fStreamSocketDescriptor == socketDescriptor && 
      ...
      )
      {
        ...
        *streamsPtr = next;
        ...
        socketDescriptor->deregisterRTPInterface(streamChannelIdToRemove);
...
Boolean RTPInterface::sendPacket(unsigned char* packet, unsigned packetSize) {
  ...
  for (tcpStreamRecord* stream = fTCPStreams; stream != NULL; stream = nextStream)
  {
    ...
    if (!sendRTPorRTCPPacketOverTCP(
        packet,
        packetSize,
				stream->fStreamSocketDescriptor,
...
)
{
  ...
  for (
    ...
    streams->fStreamSocketDescriptor->registerRTPInterface(streams->fStreamChannelId, this);
  }
}
...
void RTPInterface::stopNetworkReading() {
  ...
  for (tcpStreamRecord* streams = fTCPStreams; streams != NULL; streams = streams->fNext) {
    streams->fStreamSocketDescriptor->deregisterRTPInterface(streams->fStreamChannelId);
  }
}
...
)
{
  ...
  fprintf(stderr, "sendRTPorRTCPPacketOverTCP: %d bytes over channel %d (socket %d)\n",
	  packetSize, streamChannelId, socketDescriptor->socketNum()); fflush(stderr);
#endif
  ...
  do {
    ...
    if (!sendDataOverTCP(socketDescriptor, framingHeader, 4, False)) break;

    if (!sendDataOverTCP(socketDescriptor, packet, packetSize, True)) break;
#ifdef DEBUG_SEND
...
{
  int sendResult = send(socketDescriptor->socketNum(), (char const*)data, dataSize, 0/*flags*/);
...
#endif
      makeSocketBlocking(socketDescriptor->socketNum(), RTPINTERFACE_BLOCKING_WRITE_TIMEOUT_MS);
      sendResult = send(socketDescriptor->socketNum(), (char const*)(&data[numBytesSentSoFar]), numBytesRemainingToSend, 0/*flags*/);
...
#endif
	removeStreamSocket(socketDescriptor, 0xFF);
	return False;
      }
      makeSocketNonBlocking(socketDescriptor->socketNum());
...
SocketDescriptor::~SocketDescriptor()
{
  ...
  if (fSubChannelHashTable != NULL) {
    ...
    while ((rtpInterface = (RTPInterface*)(iter->next(key))) != NULL) {
      ...
      rtpInterface->removeStreamSocket(this, streamChannelId);
    }
    delete iter;
...
)
{
  ...
  fSubChannelHashTable->Add(
    (char const*)(long)streamChannelId,
		rtpInterface
  );
...
: fNext(next)
, fStreamSocketDescriptor(socketDescriptor)
