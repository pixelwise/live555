...
void BasicTaskScheduler::SingleStep(unsigned maxDelayTime)
{
  ...
  perform_handler(pop_next_relevant_handler(), select_result);
  // Note that we do this *after* calling a socket handler,
  // in case the triggered event handler modifies The set of readable sockets.
  handle_newly_triggered_events();
  // Also handle any delayed event that may have come due.
  fDelayQueue.handleAlarm();
}


auto BasicTaskScheduler::perform_select(struct timeval tv_timeToDelay) const -> select_result_t
{
...
struct timeval BasicTaskScheduler::get_select_wait_time(unsigned maxDelayTime)
{
  ...
  return tv_timeToDelay;
}

void BasicTaskScheduler::handle_newly_triggered_events()
{
...
void BasicTaskScheduler::perform_handler(HandlerIterator iter, select_result_t select_result)
{
  HandlerDescriptor* handler;
  while ((handler = iter.next()) != NULL)
  {
    int sock = handler->socketNum; // alias
    int resultConditionSet = select_result.condition_set(sock);
    if (
